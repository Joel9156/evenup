...
public class ExpenseService(SplitwiseDbContext db) : IExpenseService
{
    ...
    public async Task<ExpenseResult<bool>> DeleteExpenseAsync(Guid expenseId, Guid requestingUserId, CancellationToken ct = default)
    {
        ...
        db.Expenses.Remove(expense); // ExpenseShares cascade-delete via the FK configured in SplitwiseDbContext.
        await db.SaveChangesAsync(ct);
